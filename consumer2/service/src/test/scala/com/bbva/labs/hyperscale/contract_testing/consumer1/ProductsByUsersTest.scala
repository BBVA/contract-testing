...
class ProductsByUsersTest extends FlatSpec with Matchers with BeforeAndAfterAll with BeforeAndAfterEach with MockFactory {
    ...
    private def service(mergeFunction: ((JArray, JArray) => JArray)) = new ProductsByUsers(s"http://localhost:${serverUser.getLocalPort}", s"http://localhost:${serverSales.getLocalPort}") {
