...
public class SalesByUserContractTest {
    ...
    @Pact(provider = "users", consumer = "consumer1")
    public RequestResponsePact createUsersFragment(PactDslWithProvider builder) {
        ...
        DslPart body = PactDslJsonArray.arrayEachLike()
            ...
            .stringType("address", "address-1")
            .closeObject();
        return builder
            .uponReceiving("Requestubg users with address")
            .path("/api/users")
            .method("GET")
            .willRespondWith()
            .status(200)
    ...
    @Pact(provider = "sales", consumer = "consumer1")
    public RequestResponsePact createSalesFragment(PactDslWithProvider builder) {
        ...
        DslPart body = PactDslJsonArray.arrayEachLike()
            ...
            .eachLike("purchases")
            .stringMatcher("price", "\\d+(\\.\\d+)? EUR", "13 EUR");
        return builder
            .uponReceiving("Requesting sales with prices")
            .path("/api/sales")
            .method("GET")
            .willRespondWith()
            .status(200)
    ...
    public void mergeSalesByUsers() throws Exception {
        Await.result(new SalesByUsers("http://localhost:" + usersPort, "http://localhost:" + salesPort).execute(), new FiniteDuration(3, TimeUnit.SECONDS));
    }
}
